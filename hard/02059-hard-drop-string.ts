...
type ValueOf<T extends string> = T extends `${infer F}${infer R}`
  ? F | ValueOf<R>
  : never;

type DropString<S, R extends string> = S extends `${infer H}${infer T}`
  ? `${H extends ValueOf<R> ? '' : H}${DropString<T, R>}`
  : S
